...
function getCoordinateArray(nodeCount, boundingFeatures) {
	var arr = [];
	if(Array.isArray(boundingFeatures)){
		for (var i = 0; i < nodeCount; i++) {
			arr = arr.concat([randomCoordinates(boundingFeatures)]);
		}
	} else {
		const points = rand(nodeCount, boundingFeatures);
